...
按类别看：finance（从表格提取数字）在所有条件下均为 30/30；monitor
在 C=1 时为 30/30（每次都回答“需要处理”则为 15/30）。db 最弱，
几乎所有错误都是多列出了表名。BF16（未量化）也有相同倾向（C=1 时
BF16 为 20/30，本配方为 24/30），因此并非量化所致。

测量时须关闭 **prefix caching**：先在启动日志确认
`enable_prefix_caching=False`。启用后重复发送相同提示词会从缓存返回
prefill；同一提示词的第二轮速度远高于模型实际能力，因此表中不包含
这类虚高数据。详见 `SERVING-NOTES-2026-09-20.zh.md` 的测量方法。
